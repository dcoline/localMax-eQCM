...
% 
 while (currentInit <= length(sortMaxInd)) & (noNewInit == 0)
    if (sortMaxV(currentInit) < gamma * sortMaxV(1))                        %step 0
        noNewInit = 1;
    else
        if (ismember(sortMaxEdges(currentInit, 1), nodesInCluster)==0 & ... %step 1
            ...
            while addingMode == 1                                           %step 3.1
                ...
                if (c_v >= alphaN * currentDensity)                         %step 3.2
